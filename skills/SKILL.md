...
### Dispatch a message (non-blocking)

Send a message without waiting for the session to respond:

```sh
agent-bridge dispatch <sessionId> <message>
```
...
Set the model used when creating new sessions:

```sh
# View current
agent-bridge model

# Set (e.g., to avoid unstable flash model)
agent-bridge model set opencode-go/deepseek-v4-pro
```

The model is stored in `~/.config/multi-agent-bridge/config.json` and applied automatically to all new sessions.
